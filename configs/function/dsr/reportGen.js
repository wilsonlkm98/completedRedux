...
const processRegionDsr = async (data, dsrs, progStart) => {
  ...
  for (const dsr of dsrs) {
    ...
    data[dsr.region].totalVisitDays += dsr.dataValues.n_visits;
    ...
    for (const tran of dsr.transactions) {
      if (tran.status == 'Approved') {
        ...
        data[dsr.region].totalSales =
          data[dsr.region].totalSales + tran.sales
      ...
      if (tran.status == 'Approved' && tran.promo) {
        ...
        data[dsr.region].totalTransWithPromoPercent = (
          (data[dsr.region].totalTransWithPromo /
            data.total.totalTransWithPromo) *
          100
        ).toFixed(2);
      }
      data[dsr.region].totalSoldUnit = tran.dataValues.n_items
      data.total.totalSoldUnit =
        data.total.totalSoldUnit + data[dsr.region].totalSoldUnit;
      // cbTran()
    }
    ...
    if (dsr.users.length > 0) {
      ...
      data[dsr.region].activeDsrPercent = (
        (data[dsr.region].activeDsr / data[dsr.region].totalDsrs) *
        100
      ).toFixed(2);
    ...
    for (const posm of dsr.posms) {
      ...
      const key = `posm${posm.number}Deployment`;
      data[dsr.region][key]++;
    }
    let active = 0;
    let completed = 0;
    ...
    const approved = dsr.transactions.filter((e) => {
    ...
    if (dsr.transactions.length > 0 && approved.length > 0) {
      ...
      data[dsr.region].effectiveDsrPercent = (
        (data[dsr.region].effectiveDsr / data[dsr.region].totalDsrs) *
        100
      ).toFixed(2);
    }
    data[dsr.region].activeUser += active;
    data[dsr.region].completedUser += completed;
    data[dsr.region].totalScan += parseInt(dsr.dataValues.n_scans)
  ...
  for (const dsr of dsrs) {
    if (!data[dsr.code]) {
      data[dsr.code] = {
        ...
        qrcode: dsr.qrcode,
        status: dsr.status,
        visitDays: dsr.dataValues.n_visits,
        region: dsr.region,
        state: dsr.state,
        totalScan: 0,
        totalReg: 0,
    ...
    for (const tran of dsr.transactions) {
      if (tran.status == 'Approved') {
        ...
        data.total.totalTrans++;
        data[dsr.code].totalSales += tran.sales
        data.total.totalSales += tran.sales
      }
      if (tran.status == 'Approved' && tran.promo) {
        data[dsr.code].totalTransWithPromo++;
        data.total.totalTransWithPromo++;
        data[dsr.code].totalTransWithPromoPercent = (
          (data[dsr.code].totalTransWithPromo /
            data.total.totalTransWithPromo) *
          100
        ).toFixed(2);
      }
      data[dsr.code].totalSoldUnit = tran.dataValues.n_items
...
const processWeekly = async (data, events) => {
  for (const event of events) {
    ...
    if (data[`W${myweek}`]) {
      ...
      data[`W1`].totalStores = '69';
      data[`W1`].totalDDSRs = '1';
      data.Total.totalDSRs = '1';
      data.Total.totalStores = '69';
    }
  }
};
